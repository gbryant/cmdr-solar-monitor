...
DEFAULT_HOST     = 'cmdr-solar-monitor.local'
DEFAULT_PORT     = 23
DEFAULT_DB       = 'solar.db'
...
def poll_once(sock, prefix):
    # `ina <ch> stats` reads voltage and current in a single round-trip (~1 ms
    # between the two I2C reads) so both values come from the same ADC cycle.
    line = run_command(sock, f'ina {prefix} stats')
...
def main():
    ...
    while True:
        try:
            ...
            voltage, current, power = poll_once(sock, args.prefix)
            ...
            if voltage is None:
                ...
                if error_count >= 3:
                    run_command(sock, f'ina {args.prefix} init')
                    print(f'{ts}  sent ina {args.prefix} init (repeated errors)')
                    error_count = 0
            else:
                error_count = 0
                if current == 0.0 and voltage > 1000 and not reinit_sent:
                    run_command(sock, f'ina {args.prefix} init')
                    print(f'{ts}  0.0 mA — sent ina {args.prefix} init')
                    reinit_sent = True
                elif current != 0.0:
                    reinit_sent = False
                db.execute(
